...
에이전트 브라우저에 탭을 만들면 사용자 화면에 드러나지 않는다. 탭은 생기는데 보이지 않아 **사용자가 미리보기를 찾지 못한다** (실측).
미리보기는 사람이 읽어야 하므로 기본 브라우저로 띄운다.
본문을 고쳐 재생성할 때 브라우저를 다시 열면 같은 파일의 탭이 계속 쌓이고, 사용자는 오래된 탭을 읽고 판단한다.
