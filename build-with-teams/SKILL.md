...
이 스킬은 **여러 레포가 공유하는 단일 코어**다 (`~/personal/fos-skills/build-with-teams`, 글로벌 `~/.claude/skills/build-with-teams` 로 symlink).
워크플로 개선은 여기 한 곳만 고치면 전 레포에 반영된다.
레포마다 다른 부분(빌드/검증 명령·브랜치 규칙·에이전트 이름·스키마 세부·커밋 컨벤션)은 아래 오버레이로 주입한다.
