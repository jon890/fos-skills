...
**표기 검사기를 두고 있으면 본문 파일에 돌린다.** 없으면 이 항목을 건너뛴다.

```bash
~/.claude/scripts/korean-style-check.sh "$SP/body.md"
python3 ~/.claude/scripts/check-readability.py "$SP/body.md"
```

둘 다 종료 코드 0 이어야 넘어간다. 1 이면 걸린 자리를 고치고 다시 돌린다.

**만든 방법과 무관하게 이 자리에서 직접 돌린다.**
편집 훅은 편집 도구로 만든 파일만 검사한다.
Bash 의 heredoc 이나 `sed` 로 만든 본문은 훅이 돌지 않아 위반이 그대로 나간다 (실측).

검사기가 보지 못하는 축은 사람이 본다.

- **분량 구간.** 훑어 찾는 글이면 댓글에도 헤더를 쓴다. 처음부터 끝까지 읽는 짧은 댓글에만 헤더를 뺀다
- **비유.** 실제 동작이나 수치로 바꿀 자리
- **문장 성분.** 주어와 목적어가 빠져 독자가 관계를 짐작하게 되는 자리
- **중복.** 표가 이미 말하는 것을 아래 문장이 다시 쓴 자리
- **매체.** heredoc 의 escape 잔존, 압축된 표 셀

개인 문체 참조를 두고 있으면 그 점검 항목도 함께 통과한다.
