...
- `index.json`, phase 파일. 스키마·phase 구조·자체 검토·verify-task 절차는 `task-create.md` 를 따른다.
- 규모가 큰 요청은 규모 분할 점검을 통과한 여러 plan 디렉터리. 각 plan 은 자체 브랜치·PR·검증 단위이며, 실행 순서는 사용자 보고로 전달한다.
...
7단계 문서 우선 점검, 규모 분할 점검, plan별 verify-task 0 줄, 자체 검토 통과, 모호점 **0** → 각 plan 브랜치에 커밋·push(완료 후 절차).
