...
네 자리가 각각 다른 뜻이라 한 낱말로 치환하지 않았다.
규모는 「정한다」, 의도와 책임은 「판정한다」, 대기와 실패는 「구분한다」 로 적었다.

## 5.2.0

남기는 코드와 문서가 계획서를 번호로 가리키지 않게 한다.

계획서는 구현이 끝나 운영에 반영되면 제거된다.
주석, docstring, 테스트 이름, 로그 문자열, 반복 함정 문서가 번호로 가리킨 자리는 그때 깨진 참조가 된다.
executor 가 쓰지 않게 하고, docs-verifier 가 새로 남은 것을 잡는다.

**폐기를 판정하는 시간 축에 계획서 번호를 쓰지 않는다.**
계획서가 사라지면 셀 수 없고, 생성 속도가 시기마다 달라 같은 개수가 같은 기간을 뜻하지도 않는다.
마지막으로 재발한 날짜를 적고 기간으로 판정한다.

완료 마킹이 제거 허가가 아니라는 것을 밝혔다. 제거 판정은 `docs-check` 가 한다.
