...
- 검토 단계에서 반영하지 않은 발견과 그 이유. 검토를 건너뛰었으면 그 사실
...
- 어느 백엔드로 띄울지는 설정 파일의 `previewDriver` 가 정한다.
  `PREVIEW_BROWSER_DRIVER` 로 그 호출만 바꿀 수 있고, 둘 다 없으면 드라이버의 자동 감지를 따른다.
  미리보기는 사람이 읽는 화면이라 IDE 안의 탭에 떠야 하고 워크트리 대조도 되어야 하는데,
  자동화용 백엔드는 그 둘을 주지 못한다. 정하지 않으면 대조를 건너뛴다고 알린다
