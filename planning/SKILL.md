...
모든 planning은 다음 다섯 문서를 관리한다. 파일이나 디렉터리가 없으면 7단계에서 만든다.
...
7단계에서 다섯 문서를 모두 읽고 합의된 결정의 책임 문서를 판정한다. 영향을 받는 문서는 현재 상태 설명으로 실제 갱신한다. 영향이 없는 문서는 의미 없는 문구를 추가하지 않고 7단계 검토 결과에 근거를 남긴다. 장기 기술 결정이 없으면 빈 ADR을 만들지 않는다.
