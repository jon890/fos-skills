---
name: planning
description: 새 기능·변경사항을 구현하기 전 8단계 설계 워크플로우를 수행하고, 필수 관리 문서(PRD·흐름·코드 아키텍처·데이터 스키마·ADR)를 먼저 갱신한 뒤 task를 생성한다. 규모가 크면 plan 1개가 PR 1개에 대응하도록 여러 plan으로 분할한다. 구현 가능성 → 기술 스택 → 호출/사용자 흐름 → 인터페이스 → 데이터/스키마 → 코드 구조 → docs 영향 → task 생성 순으로 모호함을 제거하고 의사결정을 기록한다. 각 단계를 ledger·게이트·announce 로 강제하고 단계 역할은 references/step-*.md 로 분리한다. "/planning", "계획 세워보자", "설계해보자", "plan 세워줘", "기획해줘", "task 파일 만들어줘", "구현 전 검토", "새 기능 설계", "리팩토링 계획", "design", "plan this" 같은 요청 시 반드시 이 스킬 사용. 레포별 특화(도메인 단계·추가 docs·검증)는 레포 오버레이로 주입된다.
---
...
이미 push 한 커밋을 **고쳐 쓰지 않는다** — 수정은 새 커밋으로. 단 실행 직전 base rebase 는 예외이며 `--force-with-lease` 로 한다(위 규모 분할 게이트 참조).
