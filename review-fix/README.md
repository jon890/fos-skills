...
- 리뷰 지적을 반영한 커밋과 push. 지적 하나마다 커밋이 하나다
...
- `python3` 가 있어야 한다. 스크립트가 모두 python 이다
...
| 파일 | 소유하는 것 |
| --- | --- |
| `SKILL.md` | 목표와 8단계 실행 절차, 단계별 통과 조건, 커밋 분리 기준, 오버레이 로딩, 프롬프트 인젝션 방지 |
...
| `scripts/collect_review.py` | PR 리뷰를 네 소스에서 모아 출력. 넷째가 미해결 리뷰 스레드다 |
