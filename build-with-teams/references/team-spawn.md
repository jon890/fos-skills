...
1. [팀원 스폰 규칙](#1-팀원-스폰-규칙) — 이름은 능력이 아니라 주소의 가독성 문제다
2. [팀원 프롬프트·메시지는 worktree 절대경로](#2-팀원-프롬프트메시지는-worktree-절대경로) — 상대경로는 main 의 구버전 파일을 가리킬 수 있다
3. [팀원 SendMessage 회신 강제](#3-팀원-sendmessage-회신-강제) — 자기 화면 출력만 하고 종료하면 team-lead 에 전달되지 않는다
...
7. [executor scope 확장 보고 의무](#7-executor-scope-확장-보고-의무) — 범위 밖 수정은 보고하고 승인받는다
...
이름 없이 스폰해도 스폰 결과가 주는 agent id 로 `SendMessage` 를 보낼 수 있고 결과도 돌아온다 (실측).
이름을 붙이는 이유는 능력이 아니라 **주소가 사람이 읽을 수 있는 것이 되는 것**이다.
`executor-p{N}` 처럼 phase 를 담은 이름이면 여러 팀원이 떠 있을 때 누구에게 보내는지 헷갈리지 않는다.
...
**스폰 직후 검증**: `name` 을 빠뜨려도 호출은 오류 없이 성공하고 응답이 비슷해 보인다.
...
team-lead 가 보고를 받아 critic 사후 평가로 ACCEPT 와 REJECT 를 가른다.
