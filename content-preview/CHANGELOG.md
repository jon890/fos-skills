...
- **미리보기를 띄우는 `scripts/show-preview.sh` 를 추가하고 `orca tab create` 직접 호출을 걷어냈다.**
  `orca tab create` 는 탭을 만들지만 앞으로 가져오지 않아, 사용자가 미리보기를 찾지 못하는 일이
  실제로 났다. 빠진 단계는 `orca tab switch --focus` 였다.
  본문을 고쳐 재생성할 때마다 같은 파일의 탭이 쌓이는 문제도 있었다.
  사용자가 오래된 탭을 새 본문으로 읽고 판단할 수 있다.
  스크립트는 같은 URL 의 탭을 찾아 `orca goto` 로 갱신하고, 없을 때만 새로 만든다.
  orca 가 없으면 `open` 으로 넘어간다.
- SKILL.md 에 「화면에 띄우기」 절을 넣었다. 출력이 `갱신` 인지 `새 탭` 인지 확인하라는 지시를 담았다.
