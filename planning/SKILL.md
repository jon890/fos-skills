...
7단계에서 합의된 결정의 책임 문서를 판정한다. 영향을 받는 문서는 현재 상태 설명으로 실제 갱신하고, 영향이 없는 문서는 문구를 건드리지 않고 근거만 남긴다. 장기 기술 결정이 없으면 빈 ADR을 만들지 않는다.
...
rebase 는 **PR 생성 전에만** 한다. 이 시점엔 PR 이 없어 리뷰 코멘트가 커밋에 묶여 있지 않으므로 히스토리를 다시 써도 잃을 것이 없고, merge 를 반복할 때 쌓이는 머지 커밋도 피한다. `--force-with-lease` 는 원격이 그사이 움직였으면 실패하므로 남의 커밋을 덮지 않는다.
